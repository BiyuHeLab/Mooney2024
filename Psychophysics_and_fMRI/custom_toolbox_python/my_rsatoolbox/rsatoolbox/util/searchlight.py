...
def chunk2rdm(data_2d,events,chunk,centers,neighbors, method):
        ...
        conds = np.repeat(conds, n_rep) #hard coded for now, sorry
        sessions = np.tile(np.arange(n_rep), nCond)
        for c in chunk:
            ...
            ds = Dataset(data_2d[:, center_neighbors],
                     ...
                     obs_descriptors={'conds': conds, 'sessions': sessions},
                     channel_descriptors={'voxels': center_neighbors})
            center_data.append(ds)
        
        RDM_corr = calc_rdm(center_data, method=method,
                        descriptor='conds',cv_descriptor='sessions')
        return RDM_corr

def get_searchlight_crossnobis_RDMs(data_2d, centers, neighbors, events,sessions,
                         method='correlation', verbose=True,n_jobs = 16):
    ...
    if n_centers > 1000:
        ...
        RDM_corrs = Parallel(n_jobs=n_jobs)(delayed(chunk2rdm)(data_2d,events,chunk,centers, neighbors, method,sessions) for chunk in tqdm(chunked_center))
...
def evaluate_models_searchlight(sl_RDM, models, eval_function, method='corr', theta=None, n_jobs=16):
