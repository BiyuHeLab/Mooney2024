...
def calc_rdm(dataset, method='euclidean', descriptor=None, noise=None,
    ...
    if isinstance(dataset, Iterable):
        ...
        if method == 'euclidean':
        ...
        elif method == 'euclidean_SL':
            rdm = calc_rdm_euclidean_SL(dataset, descriptor)
            ...
            rdm = calc_rdm_crossnobis(dataset, descriptor, noise,
                                      cv_descriptor)
        elif method == 'calc_rdm_crossnobis_fullmatrix': #for permutation test
            rdm = calc_rdm_crossnobis_fullmatrix(dataset, descriptor, noise,
                                      cv_descriptor)
        elif method == 'poisson':
        ...
        if descriptor is not None and method != 'calc_rdm_crossnobis_fullmatrix':
            rdm.sort_by(**{descriptor: 'alpha'})
    return rdm

def calc_rdm2(dataset, dataset2, method='euclidean', descriptor=None, noise=None,
             cv_descriptor=None, prior_lambda=1, prior_weight=0.1):
    """
    calculates an RDM from an input dataset
...
def calc_rdm_euclidean_SL(dataset, descriptor=None):
    """
    Args:
        dataset (rsatoolbox.data.DatasetBase):
            The dataset the RDM is computed from
    ...
    for i in range(10):
        for j in range(10):
            if i == j:
                submatrix = data[i*ind:(i+1)*ind, j*ind:(j+1)*ind] #Pick submatrices (10x10) down the diagonal.
                if submatrix.shape == (8, 8): # Check if submatrix indices are valid
                    imgbyimgALL.append(submatrix)
    imgbyimg_avg = np.mean(imgbyimgALL, axis=0)
    rdm = imgbyimg_avg
    rdm = _extract_triu_(rdm) / measurements.shape[1]
    #print(rdm.shape)
    #events = np.arange(8) #change descriptor to accomodate new 8x8 matrix.
...
def calc_rdm_crossnobis_fullmatrix(dataset, descriptor, noise=None,
                        cv_descriptor=None):
    """
    ...
    if (noise is None) or (isinstance(noise, np.ndarray) and noise.ndim == 2):
        for i_fold, fold in enumerate(cv_folds):
            ...
            rdm = _calc_rdm_crossnobis_single_fullmatrix(
    ...
    rdmavg = np.mean(rdm_array, axis=0)
    return rdm
'''_build_rdms(
        rdm,
        datasetCopy,
        'calc_rdm_crossnobis_fullmatrix',
        descriptor,
        noise=noise,
        cv=cv_descriptor
    )'''
...
def calc_rdm_crosspearsons(dataset, descriptor, noise=None,
                        cv_descriptor=None):
    """
    calculates an RDM from an input dataset using Cross-nobis distance
    This performs leave one out crossvalidation over the cv_descriptor.
    ...
    if (noise is None) or (isinstance(noise, np.ndarray) and noise.ndim == 2):
        for i_fold, fold in enumerate(cv_folds):
            ...
            measurements_test, _, _ = \
                average_dataset_by(data_test, descriptor)
            rdm = _calc_rdm_pearsons_single(
                measurements_train, measurements_test)
            rdms.append(rdm)
    else:  # a list of noises was provided
        measurements = []
        variances = []
...
def calc_rdm_crossnobis_permutation (dataset, dataset2, descriptor, noise=None,
                        cv_descriptor=None):
    """
    calculates an RDM from an input dataset using Cross-nobis distance
    This performs leave one out crossvalidation over the cv_descriptor.
    ...
    ###############################
    #TESTING DATASET:
    datasetCopy2 = deepcopy(dataset2)
    
    #single_matrix_size = 21 #(7 * (7-1) / 2)
    '''for s in range(1, total_shifts):
        print('shift')
        for i in range(0, total_rows, single_matrix_size):
            subset = betamap2[i:i + single_matrix_size, :]
            shifted_subset = np.roll(subset, shift = -1, axis = 0) #shift row by 1, iteratively in this loop
            betamap2[i:i + single_matrix_size, :] =shifted_subset # remember, it's very critical that you don't assign a new variable after the shift, so you can iteratively shift down the rows!
        betamap2 = betamap2'''
    ...
    if (noise is None) or (isinstance(noise, np.ndarray) and noise.ndim == 2):
        for i_fold, fold in enumerate(cv_folds):
            data_test = datasetCopy2.subset_obs(cv_descriptor, fold)
            data_train = datasetCopy.subset_obs(
                cv_descriptor,
                np.setdiff1d(cv_folds, fold)
            )
            ...
            print(measurements_train.shape)
            print('measurements_test')
            print(measurements_test.shape)
            for p_fold in range(0,numManips):
                for q_fold in range(p_fold+1,numManips):
                    rdm = _calc_rdm_crossnobis_single_loop(
                    measurements_train[p_fold,:], measurements_test[q_fold,:], noise)
            rdms.append(rdm)
    else:  # a list of noises was provided
        measurements = []
        variances = []
        for i, i_fold in enumerate(cv_folds):
            ...
            variances.append(np.linalg.inv(noise[i]))
        for i_fold in range(len(cv_folds)):
            for j_fold in range(i_fold + 1, len(cv_folds),10):
                if i_fold != j_fold:
                    rdm = _calc_rdm_crossnobis_single(
                        measurements[i_fold], measurements[j_fold],
                        np.linalg.inv(
    ...
    return _build_rdms(
        rdm,
        datasetCopy,
        'calc_rdm_crossnobis_permutation',
        descriptor,
        noise=noise,
        cv=cv_descriptor
    )
...
def _calc_rdm_crossnobis_single_fullmatrix(meas1, meas2, noise) -> NDArray:
    rdm = np.zeros((meas1.shape[0], meas2.shape[0]))
    for i in range(meas1.shape[0]):
        for j in range(meas2.shape[0]):
            a = meas1[i,:] #meas1 is shape (70,3821)
            b = meas2[j,:] #meas2 is shape (70,3821)
            #print(a*a + b*b - 2*(a*b))
            rdm[i,j] = np.dot(a, a) + np.dot(b, b) - 2 * np.dot(a, b)
    return rdm

def _calc_rdm_pearsons_single(meas1, meas2, noise) -> NDArray:
    kernel = np.zeros((meas1.shape[0], meas2.shape[0]))
...
def _build_rdms(
    ...
    if method == 'euclidean_SL':
        rdms.descriptors['descriptor']= np.arange(8)
    return rdms
